...
const PROGRAM_ID = new PublicKey('5bvRgrP5KSvnftn2Hm7PWWfTGayCpxHy2vztRbkV2fx6');
...
async function main() {
  ...
  const secretKey = JSON.parse(fs.readFileSync('/Users/avhidotsol/.config/solana/id.json', 'utf-8'));
  ...
  await sendAndConfirmTransaction(connection, createVaultATx, [payer, vaultAKeypair]);
  ...
  await sendAndConfirmTransaction(connection, createVaultBTx, [payer, vaultBKeypair]);
  ...
  for (let i = 0; i < accountsDebug.length; i++) {
    for (let j = i + 1; j < accountsDebug.length; j++) {
      if (accountsDebug[i].pubkey.equals(accountsDebug[j].pubkey)) {
        console.error(`DUPLICATE: ${accountsDebug[i].name} and ${accountsDebug[j].name} have the same pubkey!`);
      }
    }
  }
  ...
  console.log('User Token A balance:', userTokenAInfo.amount.toString(), '(should be 900,000)');
  console.log('User Token B balance:', userTokenBInfo.amount.toString(), '(should be 900,000)');
  console.log('User LP Token balance:', userLpTokenInfo.amount.toString());
  console.log('Vault A balance:', vaultAInfo.amount.toString(), '(should be 100,000)');
  console.log('Vault B balance:', vaultBInfo.amount.toString(), '(should be 100,000)');
